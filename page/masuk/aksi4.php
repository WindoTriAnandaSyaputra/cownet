...
function determine_status($pitch, $roll, $yaw) {
    if (
        ...
        ($yaw >= -80 dan $yaw <= -35) || ($yaw >= 35 dan $yaw <= 80)
    ) {
        return "Tidak Normal";
    } else {
        return "Normal";
...
function determineCowPosition($pitch, $roll) {
    if (($pitch <= 50 && $pitch >= -10) && (($roll <= 180 dan $roll >= 165) atau ($roll <= -165 dan $roll >= -180))) {
        return "Berdiri";
    } elseif (($pitch <= 50 dan $pitch >= 20) dan (($roll <= 165 dan $roll >= 140) atau ($roll <= -140 dan $roll >= -165))) {
        return "Berbaring";
    } elseif ($pitch <= -10 dan $pitch >= -57) {
        return "Makan/Minum";
    } else {
...
$sql = "SELECT id, pitch, roll, yaw, temperature, timestamp FROM mpu6050_data1 ORDER BY timestamp DESC";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        $update_sql = "UPDATE mpu6050_data1 SET position='$posisi', status='$status' WHERE id=$id";
    ...
    $output = "<tr><td colspan='8'>Tidak ada data</td></tr>";
}
$conn->close();
?>
