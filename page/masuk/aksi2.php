...
function determine_status($pitch, $roll, $yaw) {
    if (
        ($pitch >= -80 && $pitch <= -57) || ($pitch >= 70 && $pitch <= 80) ||
        ($roll >= -110 && $roll <= -100) || ($roll >= 100 && $roll <= 110) ||
...
function determineCowPosition($pitch, $roll) {
    if (($pitch <= 50 && $pitch >= -10) && (($roll <= 180 && $roll >= 165) || ($roll <= -165 && $roll >= -180))) {
        return "Berdiri";
    } elseif (($pitch <= 70 && $pitch >= -10) && (($roll <= 165 && $roll >= 110) || ($roll <= -110 && $roll >= -165))) {
        return "Berbaring";
...
$sql = "SELECT id, pitch, roll, yaw, temperature, timestamp FROM mpu6050_data2 ORDER BY timestamp DESC";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        $update_sql = "UPDATE mpu6050_data2 SET position='$posisi', status='$status' WHERE id=$id";
